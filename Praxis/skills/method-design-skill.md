# Skill: Method Design (方法设计) — Phase 4

## 触发场景
Phase 3 Gap Review 判定 Pass，需要设计解决 Gap 的方法。

## 输入判断

| 模式 | 判断条件 | 额外输入 |
|------|---------|---------|
| **首次执行** | 不存在 `method-review.md` 且 `iteration-log.md` 无指向 P4 的 Entry | 无 |
| **P5 Revise 迭代** | 存在 `method-review.md` | `method-review.md` |
| **P8 L2 Swap 迭代** | `iteration-log.md` 最新 Entry 为 L2，目标 Phase 4 | `iteration-log.md` + 当前 `method-design.md` |
| **P8 L3 Redesign 迭代** | `iteration-log.md` 最新 Entry 为 L3，目标 Phase 4 | `iteration-log.md` + 当前 `method-design.md` |

**基础输入**：`gap-analysis.md` + `project-startup.md` + 知识库 (Methods Bank)
...
3. 重点修复逻辑跳跃、组件必要性、差异化等被标记的问题

### P8 L2 Swap 迭代

1. 读 `iteration-log.md`，定位**失败组件**
2. 读当前 `method-design.md`，确认其余组件仍然有效
...
1. 读 `iteration-log.md`，理解整体失败原因和已排除方案
2. 读当前 `method-design.md` 作为参考
3. 在已排除约束下**重新设计方法框架**
4. 保留被标记为"已验证可行"的组件/设计
...
- 帮助构建理论论证
- 迭代时：先读 iteration-log.md 确认失败原因和约束，再决定改动范围
...
## 完成后
提示用户：Method Design 完成，建议执行 `/review method` 进行独立审查。
