# Skill: Experiment Design (实验设计) — Phase 6

## 触发场景
Phase 5 Method Review 判定 Pass，需要设计验证体系。

## 输入判断

| 模式 | 判断条件 | 额外输入 |
|------|---------|---------|
| **首次执行** | 不存在 `experiment-review.md` | 无 |
| **P7 Revise 迭代** | 存在 `experiment-review.md` | `experiment-review.md` |
...
2. 针对性修改：补充遗漏实验、调整 baseline、修正 metrics 等
...
## 完成后
提示用户：Experiment Design 完成，建议执行 `/review experiment` 进行独立审查。
