# Skill: Gap Discovery (研究空白发现) — Phase 2

## 触发场景
项目已完成 Phase 1（`project-startup.md` 就绪），需要系统性发现研究空白。

## 输入判断

Agent 首先检查项目目录中的文件，判断本次执行属于哪种模式：

| 模式 | 判断条件 | 额外输入 |
|------|---------|---------|
| **首次执行** | 不存在 `gap-review.md` 且不存在 `iteration-log.md` | 无 |
| **P3 Revise 迭代** | 存在 `gap-review.md` | `gap-review.md`（审查意见） |
| **P8 L4 Pivot 迭代** | 存在 `iteration-log.md` 且最新 Entry 的目标 Phase 为 Phase 2 | `iteration-log.md` + 当前 `gap-analysis.md` |
...
2. 对每条 Revise 意见，定位 `gap-analysis.md` 中的对应段落
3. 针对性修改，**不从零开始**
4. 重点修复被标记为 Revise/Block 的审查维度
5. 更新 `gap-analysis.md`

### P8 L4 Pivot 迭代

1. 读完整 `iteration-log.md`，理解：
   - 所有已排除的 Gap 方向
   - 为什么之前的方向走不通
   - 建议的新方向（如有）
2. 读当前 `gap-analysis.md` 作为参考（了解之前选的什么 Gap）
...
## 完成后
提示用户：Gap Discovery 完成，建议执行 `/review gap` 进行独立审查。
